...
export default function Navbar() {
  ...
  const isActive = (p) =>
    pathname === p ? 'shadow-sm text-black after:w-3/4' : 'text-white';

  return (
    <>
      <nav
        className={`sticky top-10 mx-14 mb-6 z-10 bg-white/90 backdrop-blur-2xl shadow-[0_1px_10px_rgba(0,0,0,0.05)] rounded-none transition-all duration-500 ease-in-out ${
          !isHover &&
      ...
      >
        <div
          className={`mx-auto  h-20 flex justify-between items-center ${
            isHover ? 'max-w-7xl px-4 sm:px-6 lg:px-8' : 'px-5'
          } `}
        >
          {/* Logo + Brand */}
          <Link href='/' className='flex items-center gap-3'>
            ...
            <span
              className={`text-2xl font-extrabold tracking-tight text-blackGray text-shadow-amber-100 text-shadow-md ${
                !isHover && 'hidden'
              }`}
            >
              TaxiBook
          ...
          <button
            ...
            className={`md:hidden !text-blackGray text-xl ${
              !isHover && 'hidden'
            }`}
